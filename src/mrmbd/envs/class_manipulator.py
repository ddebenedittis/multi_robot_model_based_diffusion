...
class RRPRSingleEnv:
    def __init__(self, dt=0.001):
        ...
        self.g0_num = jnp.array([0.0, 0.0, -9.81])
    ...
    def rrpr_dynamics(self, x, u):
        ...
        B = B_func_jitted(
            q, self.m_num, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
        )
        C = C_func_jitted(
            q, dq, self.m_num, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
        )
        G = G_func_jitted(
            q, self.m_num, self.g0_num, self.L1_num, self.L2_num, self.L3_num, self.L4_num
        )
        G = G.squeeze()
    ...
    def get_rewards(self, q, u):
        ...
        r_q_goal = 0.5 * (1.0 - joint_error / (joint_error_0 + 1e-6)) + 0.5 * (
            1.0 - jnp.abs(linear_error) / (jnp.abs(linear_error_0) + 1e-6)
        )

        # End-effector error (normalized to initial error)
        T_curr, *_ = forward_kinematics_rrpr_jax(
            pos, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
        )
        ee_pos = T_curr[:3, 3]

        T_goal, *_ = forward_kinematics_rrpr_jax(
            self.qf[:4], self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
        )
        ee_goal = T_goal[:3, 3]

        err = jnp.linalg.norm(ee_goal - ee_pos)

        T_start, *_ = forward_kinematics_rrpr_jax(
            self.q0[:4], self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
        )
        ...
        tau_required = G_func_jitted(
            pos, self.m_num, self.g0_num, self.L1_num, self.L2_num, self.L3_num, self.L4_num
        )
    ...
    def render(
        ...
        r_terms: jnp.ndarray = None,
        tag: str = "",
        out_dir: str = "results/manipulator_diffusion",
    ):
        ...
        plot_four_panel(
            ...
            outfile=f"{out_dir}/tau_all_{tag}",
        )
        ...
        if r_terms is not None:
            ...
            plot_four_panel(
                ...
                outfile=f"{out_dir}/reward_terms_{tag}",
            )
            ...
            plt.tight_layout()
            plt.savefig(f"{out_dir}/reward_total_{tag}.png", dpi=300)
            plt.savefig(f"{out_dir}/reward_total_{tag}.pdf")
            plt.close()
        ...
        for t in range(T):
            q = q_seq[t]
            T_curr, *_ = forward_kinematics_rrpr_jax(
                q, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
            )
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"{out_dir}/ee_error_{tag}.png", dpi=300)
        plt.close()
        ...
        def update(frame):
            ...
            T04, T01, T12, T23, T34 = forward_kinematics_rrpr_jax(
                q, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
            )
            T02 = T01 @ T12
            T03 = T02 @ T23
            ...
            T_goal, *_ = forward_kinematics_rrpr_jax(
                qf, self.L1_num, self.L2_num, self.L3_num, self.L4_num, self.D2_num
            )
            ee_goal = np.array(T_goal[:3, 3])
