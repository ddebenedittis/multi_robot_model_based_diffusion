...
def antipodal_positions(n, radius):
    ...
    x0_xy = jnp.stack([radius * jnp.cos(angles), radius * jnp.sin(angles)], axis=1)
    xg_xy = -x0_xy
...
class MultiCar2d:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        q_new = jax.vmap(rk4, in_axes=(None, 0, 0, None))(car_dynamics, q, action, self.dt)

        reward, _ = self.get_rewards(q_new, action)

        return state.replace(
            pipeline_state=q_new, obs=q_new, reward=reward, done=jnp.zeros((self.n,))
        )
    ...
    def get_rewards(self, q_all, u_all):
        def single_reward(k, q, u_k):
            ...
            r_total_check = (
                ...
                + r_obstacles
                + r_backward
                + self.wt * r_orient_final
            )
            r_terms = jnp.array(
                [r_goal, r_safe, r_form, r_control, r_obstacles, r_backward, r_total_check]
            )
            return r_total_check, r_terms

        r_total_all, r_terms_all = jax.vmap(single_reward, in_axes=(0, 0, 0))(
            jnp.arange(self.n), q_all, u_all
        )
    ...
    def render(
        ...
        if self.formation_shift:
            ...
            ax.add_patch(
                plt.Circle(
                    c0, self.radius, color="gray", linestyle="--", fill=False, lw=0.6, alpha=0.5
                )
            )
            ax.add_patch(
                plt.Circle(
                    cg, self.radius, color="black", linestyle="--", fill=False, lw=0.6, alpha=0.5
                )
            )
