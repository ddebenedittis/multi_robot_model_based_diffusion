...
plt.xlabel("Time [step]"); plt.ylabel("Minimum distance [m]")
plt.grid(True, linestyle="-", alpha=0.6); plt.tight_layout()
plt.savefig(f"{path}/plot_min_distance_{ecd_tag}_{form}.png"); plt.close()
...
plt.xlabel("Robot"); plt.ylabel("Error [m]")
plt.grid(True, linestyle="-", alpha=0.6); plt.tight_layout()
plt.savefig(f"{path}/plot_goal_errors_{ecd_tag}_{form}.png"); plt.close()
...
plt.savefig(f"{path}/plot_rewards_{ecd_tag}_{form}.png"); plt.close()
...
plt.xlabel("Time [step]"); plt.ylabel("Error [m]")
plt.grid(True, linestyle="-", alpha=0.6); plt.legend(); plt.tight_layout()
plt.savefig(f"{path}/plot_goal_error_over_time_{ecd_tag}_{form}.png"); plt.close()
...
def save_static_diffusion_images(trajectories_all, trajectories_denoised, env, path, cmap):
    ...
    for tag, idx in steps.items():
        ...
        for i in range(n):
            ...
            ax.plot(gx, gy, marker='s', color=palette[i], markersize=5.5, markeredgewidth=0.8, zorder=5)
            ax.plot(traj_opt[:, i, 0], traj_opt[:, i, 1], color=palette[i], linewidth=2)
            ax.plot(traj_opt[0, i, 0], traj_opt[0, i, 1], "o", color=palette[i], markersize=4)
        ...
        legend_elements = [
            ...
            Line2D([0], [0], marker='o', color='w', markerfacecolor='k', markersize=5, label='Initial position'),
            Line2D([0], [0], marker='s', color='w', markerfacecolor='k', markersize=5, label='Final position'),
        ]
