...
class CranePendulumEnv:
    ...
    def render(self, x_traj, U=None, rewards=None, r_terms=None, tag=""):
        ...
        axs[0].plot(t, theta); axs[0].set_ylabel("theta (rad)")
        axs[1].plot(t, dtheta); axs[1].set_ylabel("dtheta/dt (rad/s)")
        axs[2].plot(t, x); axs[2].set_ylabel("x (m)")
        axs[3].plot(t, dx); axs[3].set_ylabel("dx/dt (m/s)")
        axs[3].set_xlabel("Time (s)")
        plt.tight_layout(); plt.grid(True)
        plt.savefig(f"results/crane/figs/states_{tag}.png"); plt.close()
        ...
        if U is not None:
            ...
            plt.plot(t_u, 50 * U)
            plt.title(f"Control force [{tag}]"); plt.xlabel("Time (s)"); plt.ylabel("F (N)")
            plt.grid(True); plt.tight_layout()
            plt.savefig(f"results/crane/figs/control_{tag}.png"); plt.close()
        ...
        if r_terms is not None:
            ...
            axs[0].plot(t, r_goal, color='tab:green'); axs[0].set_ylabel("r_goal"); axs[0].set_title("Goal tracking"); axs[0].grid(True)
            axs[1].plot(t, E_kin, color='tab:orange'); axs[1].set_ylabel("E_kin"); axs[1].set_title("Kinetic energy"); axs[1].grid(True)
            axs[2].plot(t, E_pot, color='tab:red'); axs[2].set_ylabel("E_pot"); axs[2].set_title("Potential energy"); axs[2].set_xlabel("Time (s)"); axs[2].grid(True)
            plt.tight_layout(rect=[0, 0, 1, 0.96])
            plt.savefig(f"results/crane/figs/r_terms_{tag}.png"); plt.close()
            ...
            plt.xlabel('Time [s]'); plt.ylabel('Energy [J]'); plt.grid(True); plt.legend(fontsize=11)
            plt.tight_layout()
            plt.savefig(f"results/crane/figs/energies_{tag}.png", dpi=300); plt.close()
        ...
        if rewards is not None:
            ...
            plt.title(f"Total reward [{tag}]"); plt.xlabel("Time (s)"); plt.ylabel("Reward")
            plt.grid(True); plt.tight_layout()
            plt.savefig(f"results/crane/figs/reward_{tag}.png"); plt.close()
        ...
        plt.xlabel("Time (s)"); plt.ylabel("e_theta (rad)")
        plt.grid(True); plt.legend(); plt.tight_layout()
        plt.savefig(f"results/crane/figs/error_theta_{tag}.png"); plt.close()
